...
public class ExcelUtility
{
	...
	public void ConvertToTableTxt(string txtPath, bool isEncrypt, Encoding encoding)
    {
        //判断Excel文件中是否存在数据表
        if (mResultSet.Tables.Count < 1)
            return;
        ...
        for (int i = 0; i < rowCount; i++)
        {
            ...
            for (int j = 0; j < colCount; j++)
            {
				bool bAddTable = (j < colCount - 1);

				//使用","分割每一个数值
				lineBuidler.Append(bAddTable ? (mSheet.Rows[i][j] + "\t") : mSheet.Rows[i][j]);
            }

            //判断是否空行
            string lineContent = lineBuidler.ToString();
            lineContent = Regex.Replace(lineContent, @"\s", "");
            if (lineContent.Length > 0)
            {
                stringBuilder.Append(lineBuidler.ToString());
            }
        ...
        using (FileStream fileStream = new FileStream(txtPath, FileMode.Create, FileAccess.Write))
        {
            using (TextWriter textWriter = new StreamWriter(fileStream, encoding))
            {
				string szFinalContent = stringBuilder.ToString();
				if(isEncrypt)
                {
					szFinalContent = CEncryptHelper.AesEncryptTBL(szFinalContent);
                }

				textWriter.Write(szFinalContent);
            }
        }
    }
