...
public class CSystemInfoConst
{
    ...
    public const string SPECIALSOUND = "specsound";     //特殊声音
    public const string BROADMODEL = "broadmodel";      //提示音模板
    ...
    public const string G_SHADOW = "g_shadow";        //阴影
    public const string G_ANTI = "g_anti";            //抗锯齿
    public const string G_FILTER = "g_filter";        //后期效果
    public const string G_FULLCOLORBG = "g_fullcolorbg";        //纯色背景模式
    public const string VUTBERCODE = "vcode";    //主播身份码
    public const string ADDSEAT = "addseat";    //加座位
}
...
public class CSystemInfoMgr
{
    ...
    void CreatNewSystemFile()
    {
        ...
        SaveAudioSet(50);
        SaveBgmSet(30);
        SetInt(CSystemInfoConst.SPECIALSOUND, 1);   //默认开启角色/船音效
        SaveBroadModel((int)CConfigFishBroad.EMBroadModel.Girl1);

        if (Application.platform == RuntimePlatform.WindowsEditor ||
           Application.platform == RuntimePlatform.WindowsPlayer)
        {
            //Resolution pCurResolution = Screen.resolutions[Screen.resolutions.Length - 1];
            SetResolution(675, 900, false);
        }

        SetFPS(40);
        SetInt(CSystemInfoConst.G_SHADOW, 0);
        SetInt(CSystemInfoConst.G_ANTI, 1);
        SetInt(CSystemInfoConst.G_FILTER, 0);
        SetInt(CSystemInfoConst.G_FULLCOLORBG, 0);

        SetBool(CSystemInfoConst.ADDSEAT, false);
    ...
    public void SaveBroadModel(int value)
    {
        pSaveData.SetInt(CSystemInfoConst.BROADMODEL, value);
    }
    ...
    {
        pSaveData.SetInt(key, value);
    }

    public void SetBool(string key, bool value)
    {
        pSaveData.SetBool(key, value);
    }
