...
public class CAppPathMgr
{
    ...
    //本地排行榜记录文件路径
    public static string LOCALRankSAVEDATA_DIR
    {
        get
        {
            if (Application.platform == RuntimePlatform.WindowsEditor
            || Application.platform == RuntimePlatform.WindowsPlayer)
                return Application.dataPath + "/../../fishranklist/";
            else if (Application.platform == RuntimePlatform.Android
                || Application.platform == RuntimePlatform.IPhonePlayer)
                return Application.persistentDataPath + "/SaveData/";
            else
    ...
    //本地排行榜记录文件路径
    public static string LOCALTreasureRankSAVEDATA_DIR
    {
        get
        {
            if (Application.platform == RuntimePlatform.WindowsEditor
            || Application.platform == RuntimePlatform.WindowsPlayer)
                return Application.dataPath + "/../../fishtreasurelist/";
            else if (Application.platform == RuntimePlatform.Android
                || Application.platform == RuntimePlatform.IPhonePlayer)
                return Application.persistentDataPath + "/SaveData/";
            else
    ...
    //房间VS设置存档文件
    public static string SaveFile_RoomVSConfig = "RoomVSConfig.dat";
