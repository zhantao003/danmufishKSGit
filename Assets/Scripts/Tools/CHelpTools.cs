...
public class CHelpTools
{
    ...
    public static bool bCheckLerpCurByDay(long nlTime)
    {
        ...
        DateTime checkTime = GetDateTimeMilliseconds(nlTime);
    ...
    public static string GetGoldSZ(long money, string tail = "f2")
    {
        string szGold = string.Empty;

        if (money >= 1000000000)
        {
            float nMoney = (float)money * 0.00000001f;
            szGold = nMoney.ToString(tail) + "B";
        }
        else if (money >= 1000000)
        {
            float nMoney = (float)money * 0.000001f;
            szGold = nMoney.ToString(tail) + "M";
        }
        else if(money >= 1000)
        {
            float nMoney = (float)money * 0.001f;
            szGold = nMoney.ToString(tail) + "K";
        }
        else
        {
    ...
    /// </summary>
    /// digits:保留几位小数
    public static float FloatRound(float value, int digits = 1)
    {
    ...
    /// <returns></returns>
    public static bool CheckAuctionByUID(string uid)
    {
        bool bAuction = false;
    ...
    public static GameObject[] SphereCheck(Vector3 center, float radius, LayerMask lMsk, bool showRange = false)
    {
        ...
        if (showRange)
        {
            ...
            objCube.GetComponent<Collider>().enabled = false;
            objCube.transform.position = center;
