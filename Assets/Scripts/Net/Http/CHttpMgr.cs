...
public class CHttpMgr : MonoBehaviour {
    ...
    public void SendHttpMsg(string szRequestType, INetEventHandler handler, CHttpParam pParams = null, int reconnect = 0, bool engryt = false)
    {
        ...
        if (pParams == null)
        {
            CHttpMgr.Instance.SendDirectRequest(
                szFinalUrl + szRequestType,
                handler,
                reconnect,
                engryt);
    ...
    /// 发送请求（带用户的登录token)
    /// </summary>
    public void SendHttpMsgWithToken(string szRequestType, CHttpParam pParams = null)
    {
        if (CHttpMgr.Instance == null)
        {
            Debug.LogError("None Http GameObject");
        ...
        if (pParams == null)
        {
            //pParams = new CHttpParam(new CHttpParamSlot("autoLoginSessionKey", CUserMgr.Instance.pLoginInfo.autoLoginSessionKey));

            CHttpMgr.Instance.SendRequest(szFinalUrl +
                szRequestType,
                pParams, pHandlerMgr.GetHandler(szRequestType));
        }
        else
        {
            //pParams.AddSlot(new CHttpParamSlot("autoLoginSessionKey", CUserMgr.Instance.pLoginInfo.autoLoginSessionKey));
