...
public class FSMUnitRPGShootBoss : FSMUnitBase
{
    ...
    public override void OnFixedUpdate(object obj, float delta)
    {
        if (emCurShootState == EMShootState.Jump)
        {
            ...
            else
            {
                ...
                //判断钱够不够
                bool isCoinOK = true;
                int nZiBengPrice = CGameColorFishMgr.Ins.pStaticConfig.GetInt("滋崩价格") * 2;

                if (DateTime.Now.Year == 2022 &&
                   DateTime.Now.Month == 11 &&
                   DateTime.Now.Day == 11)
                {
                    nZiBengPrice = CGameColorFishMgr.Ins.pStaticConfig.GetInt("滋崩打折价格") * 2;
                }
    ...
    void CreateGun()
    {
        ...
        CResLoadMgr.Inst.SynLoad("Gun/GunRPG", CResLoadMgr.EM_ResLoadType.CanbeUnloadAssetbundle,
            delegate (UnityEngine.Object res, object data, bool bSuc)
            {
                ...
                pGun.Init(pUnit.uid, pUnit.pInfo.GetProAddValue(EMAddUnitProType.ZibengToRPG), pUnit.nAtkIdx, tranHitPos);
            });
    }
}
