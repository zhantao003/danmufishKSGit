...
public class DCmdSpecialDarkDuel : CDanmuCmdAction
{
    public override void DoAction(CDanmuChat dm)
    {
        ...
        {
            UIToast.Show((CTBLHandlerFucAble.Ins.GetInfo((int)EMFuncAbleType.Duel).nSecenLv + 1) + "级渔场开放海斗功能");
            return;
        }
    ...
    public void Duel(string content, CPlayerUnit player)
    {
        ...
        try
        {
            ...
            {
                Debug.Log("异常海斗弹幕:" + content);
                return;
            }
            long nlTargetPrice = 0;
            if (!long.TryParse(szContent, out nlTargetPrice))
            {
                Debug.Log("异常的海斗价格：" + szContent);
                return;
            }
            if (nlTargetPrice < CGameColorFishMgr.Ins.pStaticConfig.GetInt("海斗门票最小价格"))
            {
                UIToast.Show("海斗门票价格过低");
                return;
            }
            ...
            if (bDuelCoinMaxTupo)
            {
                if (nlTargetPrice > CGameColorFishMgr.Ins.pStaticConfig.GetInt("海斗门票最大价格2"))
                {
                    UIToast.Show("海斗门票价格过高");
                    return;
                }
            }
            else
            {
                if (nlTargetPrice > CGameColorFishMgr.Ins.pStaticConfig.GetInt("海斗门票最大价格"))
                {
                    UIToast.Show("海斗门票价格过高");
                    return;
                }
